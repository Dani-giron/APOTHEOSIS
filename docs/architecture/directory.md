...
**GEXF (Graph Exchange XML Format).** An XML-based file format for representing graph structures, consumed by visualization tools such as Gephi. The `export::gexf::draw()` function (`src/export/gexf.rs`) produces one GEXF file per HNSW layer with the naming pattern `<stem>_layer<N>.gexf`. The `gexf` crate is used to generate these files.
