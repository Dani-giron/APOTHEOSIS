    ...
    F: FeatureType + Clone,
{
    ...
    fn search_layer_knn(
        ...
        while let Some(candidate) = candidates.pop() {
            ...
            //println!("[S0] Candidates to visit: {}: | Enter point is: {}", candidates.len(), enter_point);
            /* 
            for (i, candidate) in candidates.iter().enumerate() {
                println!("  Candidate {}: feature = {:?}. Neighbors: {:?}", i, self.features[candidate.feature_index], candidate.neighbors.len());
            }
            */
            ...
            for (neighbor, _) in candidate.neighbors.iter() {
                ...
                if visited_neighbors.insert(neighbor_node.feature_index) {
                    ...
                    //println!("[S1] Visiting candidate's neighbor: {}. Score is {}", neighbor, score);
                    ...
                    //println!("[S2] Discard neighbor: {} because is already visited", neighbor);
                }
                
            }
        }
        //println!("[S6] Search time: {:?}. Candidates explored: {:?} / Neighbors explored: {:?}",time_start.elapsed(), self.candidates_explored, self.neighbors_explored );
